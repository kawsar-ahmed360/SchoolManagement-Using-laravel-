...
@section('content')
             ...
             <div class="card">
                <div class="card-body">
                             ...
                             <h4 class="card-title">Fee Ammount Update  <a href="{{ route('FeeAmmountview') }}" style="margin-left: 8px;" class="btn btn-success btn-sm"><i class="fa fa-plus-circle"></i>fee ammount list</a></h4>
                   
                      
                   <form action="{{ route('FeeAmmountupdate',$edite[0]->fee_category_id) }}" method="post" accept-charset="utf-8">
                      @csrf
                             ...
                             <div class="form-group col-md-4">
                                  ...
                                  <select class="form-control" name="fee_category_id" id="fee_category_id">
                                   ...
                                   @foreach($feecategory as $f)
                                     <option value="{{ $f->id }}" {{ ($edite[0]->fee_category_id == $f->id)?"selected":"" }}>{{ $f->fee_category }}</option>
                                   @endforeach
                                  </select>
                            ...
                            <div class="delete_whole_extra_item">
                                  ...
                                  <select class="form-control" name="class_id[]" id="class_id">
                                    <option disabled="" selected="">---Select Class--</option>
                                     @foreach($class as $c)
                                           <option value="{{ $c->id }}" {{ ($e->class_id == $c->id)?"selected":"" }}>{{ $c->class_name }}</option>}
                                  ...
                                  <div class="form-group col-md-5">
                                  <label>Ammount</label>
                                   <input type="text" class="form-control" name="ammount[]" value="{{ $e->ammount }}" id="ammount" placeholder="Enter Ammount.........">
                      ...
                      <button type="submit" class="btn btn-success btn-sm">All Update</button>
                      </div>
